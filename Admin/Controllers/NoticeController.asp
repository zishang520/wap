...
@@  .;irrrrriiii;,    @@@@@@@@@@@@@@@@@@@@@@@@@@@@
*/
NoticeController = IController.create(auth());
Mo.assign('method',Mo.Method);//注入单前控制器名称，以在模板判断是否选中
//默認Action
//內容列表頁
NoticeController.extend("Index",function(){
	var data = M("Notice","id").limit(F.get.int("page", 1),10).orderby("id desc").select();
	this.assign('data',data);
	this.display("Notice:Index");
});
//內容創建
NoticeController.extend("Create",function(){
	var rc = M("Class","id").where('grap>1').orderby("did asc").select();
	rc.assign('data');
	this.display("Notice:Create");
});
//內容顯示
NoticeController.extend("Show",function(){
	var id=F.all("id");
	if (F.string.exp(id,/^\d+$/)!=id) {
		F.session('__error','id格式不正确');
		F.goto(Mo.U('Notice/Index'));
		F.exit();
	}
	var str="id=" + id;
	var rs = M("Notice","id").where(str).orderby("id DESC").select();
	if (rs.count()==0) {
...
NoticeController.extend("Stroe",true,function(){
	var title=F.encodeHtml(F.post("title"));
	if (is_empty(title)) {
		F.session('__error','内容标题不能为空');
		F.goto(Mo.U('Notice/Create'));
		F.exit();
	}
	...
	M("Notice","id").Insert(insertstr);
	F.goto(Mo.U('Notice/Index'));
});
//內容編輯
NoticeController.extend("Edit",function(){
	var id=F.all("id");
	if (F.string.exp(id,/^\d+$/)!=id) {
		F.session('__error','id格式不正确');
		F.goto(Mo.U('Notice/Index'));
	...
	var str="id=" + id;
	var data = M("Notice","id").where(str).orderby("id DESC").select();
	if (data.count()==0){
		F.session('__error','选择的内容不存在');
		F.goto(Mo.U('Notice/Index'));
		F.exit();
...
//內容更新地址，描述同上
NoticeController.extend("Update",true,function(){
	var id=F.post("id");
	if (F.string.exp(id,/^\d+$/)!=id) {
		F.session('__error','id格式不正确');
		F.goto(Mo.U('Notice/Index'));
	...
	M("Notice","id").where(str).Update(updatestr);
	var page=F.get('page');
	if(!is_empty(page)){F.goto(Mo.U('Notice/Index','page='+page));}else{F.goto(Mo.U('Notice/Index'));}
});
//内容删除
NoticeController.extend("Del",true,function(){
	var str="id=" + F.post("id");
	M("Notice","id").where(str).Delete();
	var page=F.get('page');
	if(!is_empty(page)){F.goto(Mo.U('Notice/Index','page='+page));}else{F.goto(Mo.U('Notice/Index'));}
});
</script>