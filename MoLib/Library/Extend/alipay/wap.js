...
function get_pendding_sign_data(src,charset){
	...
	F.foreach(src,function(key, value){
		if(value=="")return;
		returnValue += key + "=" + (charset ? encoding.encodeURIComponent(value,charset) : value) + "&";
	});
...
function hex2base64(src){
	return base64.e(encoding.hex.parse(src));
}
function base642hex(src){
	return encoding.hex.stringify(base64.d(src));
}
var $alipay = IClass.create(function(charset, partner, key){//商家ID key
...
$alipay.extend("notify",function(){
	if(F.get("sign") == F.md5_bytes(
		encoding[this.config._input_charset=="utf-8"?"utf8":"gbk"].getByteArray(
