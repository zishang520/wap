...
function $qqwry(ip){
	...
	if(!VBS.include(__dirname + "\\qqwry.vbs")) return;
	var $base = VBS.require("QQWry","UseCode",true,"QQWryFile",cfg.path),
		location="",
		address="";
	try{
		$base.QQWry(ip);
	...
	if(location!="") location = encoding.gbk.getString(encoding.hex.parse(location));
	if(address!="") address = encoding.gbk.getString(encoding.hex.parse(address));
