	...
	var delegateFn = function(element, event, selector, callback) {
		return function(e) {
			...
			for (; target && target !== document; target = target.parentNode) {
				...
				if (~['click', 'click', 'doubletap', 'longtap', 'hold'].indexOf(event) && (target.disabled || target.classList.contains('mui-disabled'))) {
					break;
				}
				var matches = {};
			...
			$.each(handlerQueue, function(index, handler) {
				...
				if (event === 'click' && (tagName !== 'INPUT' && tagName !== 'TEXTAREA' && tagName !== 'SELECT')) {
					e.preventDefault();
	...
	$.fn.on = function(event, selector, callback) { //仅支持简单的事件委托,主要是tap事件使用，类似mouse,focus之类暂不封装支持
		return this.each(function() {
			...
			if (isAddEventListener) {
				...
				if (event === 'click') { //TODO 需要找个更好的解决方案
		...
		if (row) {
			if (self.sliderActionClass) {
				...
				if (self.searchActionClass) {
					...
					self.searchAction.addEventListener('click', function(e) {
						$.focus(self.element);
						e.stopPropagation();
					});
				}
				...
				if (self.clearActionClass) {
					...
					self.clearAction.addEventListener('click', function(event) {
...
(function($) {
	...
	var tapEventName = touchSupport ? 'click' : 'click';
